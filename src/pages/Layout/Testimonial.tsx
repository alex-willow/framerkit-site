// src/pages/Layout/Testimonial.tsx
...
type ComponentItem = {
  ...
  url: string;
  type: "free" | "paid";
  // section не нужен — все данные из testimonial.json
};
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setItems(json.testimonial || []);
        setLoading(false);
      } catch (err) {
        setError("Не удалось загрузить компоненты Testimonial");
        setLoading(false);
      }
    };
    load();
