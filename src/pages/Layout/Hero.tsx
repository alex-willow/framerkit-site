...
const DATA_URL = "https://raw.githubusercontent.com/alex-willow/framerkit-data/main/hero.json";
const CACHE_KEY = `remote:${DATA_URL}`;
const DATA_KEY = "hero" as const;
...
export default function HeroPage({ isAuthenticated, setIsSignInOpen }: HeroPageProps) {
  ...
  return (
    <div id="hero-page" style={{ padding: 0, scrollMarginTop: "64px" }}>
      ...
      {/* 🔥 SEO META TAGS */}
      <SEO
        title="Hero Section Components for Framer"
        description="Eye-catching hero section components for Framer. Light & dark themes, wireframe & design modes. Copy-paste ready hero banners for landing pages and websites."
        keywords="framer hero section, landing page hero, hero banner, call to action section, framer ui kit, copy paste hero, responsive hero"
        image="/og-hero.jpg"
        canonical="https://www.framerkit.site/layout/hero"
      />

      {/* 🔥 H1 для поисковиков (визуально скрыт, но индексируется) */}
      <h1 className="sr-only">Hero Section Components for Framer — Landing Page Banners</h1>
      ...
      <div className="component-page-header">
        <nav className="component-breadcrumb">
          ...
          <span className="breadcrumb-current">Hero</span>
        </nav>
        <h2 className="component-page-title">Hero Components</h2>
        <p className="component-page-description">
          Use hero sections to communicate your offer in the first screen. Start with one clear headline, one supporting line, and one primary action before adding extras.
        </p>
      </div>
      ...
      <div className="gallery-scroll-area" ref={galleryRef}>
        {loading ? (
          ...
          <div className="empty-message">No hero components available for the selected theme</div>
        ) : (
              ...
              return (
                ...
                >
                  <div className="cardImage">
                    ...
                    <img 
                      src={displayImage} 
                      alt={`${item.title} - ${isWireframeMode ? 'Wireframe' : 'Design'} hero section component for Framer`} 
                      loading="lazy" 
                    />
                  </div>
